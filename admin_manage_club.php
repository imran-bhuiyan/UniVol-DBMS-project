...
if (isset($_POST['submit'])) {
    ...
    $club_input = $_POST['club_id'];

    
    $club_input = mysqli_real_escape_string($conn, $club_input);

   
    if (is_numeric($club_input)) {
        
        $query = "SELECT id, name FROM club WHERE id = '$club_input'";
    } else {
        
        $query = "SELECT id, name FROM club WHERE name LIKE '%$club_input%'";
    }
...
<body>
    ...
    <form action="admin_manage_club.php" method="POST">
        ...
        <div class="relative">
            ...
            <input type="search" id="default-search"
                ...
                placeholder="Search by club id or name " name="club_id" value="<?php echo $club_input; ?>">
            <button type="submit"
                ...
                <?php
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td class='px-6 py-4'><a href='admin_edit_club.php?club_id=" . $row['id'] . "' class='font-medium text-blue-600 dark:text-blue-500 hover:underline'> Edit</a></td>";
                            echo "</tr>";
                        }
                    } else {
