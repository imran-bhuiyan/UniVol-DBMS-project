...
$errors = array('id'=>'','name'=>'','email'=>'','contact_num'=>'','type'=>'','password'=>'');
...
if (empty($_POST['id'])) {
    ...
    //check type
        if (empty($_POST['type'])) {
        $errors['type'] = 'Please select a type (Faculty/Varsity) <br />';
        } elseif (!preg_match('/^(Faculty|Varsity)$/', $_POST['type'])) {
             $errors['type'] = 'Invalid type selected <br />';
         }     else {
                $type = $_POST['type'];
             }
        ...
        <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 max-w-2xl mx-auto" action="clubSignup.php"
            ...
            <div class="mb-4">
                ...
                <select
                    ...
                    <option value="Faculty" <?php if ($type == 'Faculty') echo 'selected="selected"'; ?>>Faculty
                    </option>
                    <option value="Varsity" <?php if ($type == 'Varsity') echo 'selected="selected"'; ?>>Varsity
                    </option>
                </select>
                <p class="text-red-500 text-xs italic"><?php echo $errors['type']; ?></p>
            </div>
