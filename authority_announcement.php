...
$authority_id = $_SESSION['autho_id'];
...
if (isset($_POST['submit'])) {
    ...
    $checkAuthority = "SELECT * from event_details where id = '$event_id' and faculty_id = '$authority_id' ";
    $checkAuthority_result =  mysqli_query($conn, $checkAuthority);

   
    
    if (mysqli_num_rows($checkAuthority_result) === 0) {
    ...
    if (empty(array_filter($errors))) {
        ...
        $sql = "INSERT INTO announcements (title, event_id, content,club_id,authority_id)
                VALUES ('$title', '$event_id', '$details','0','$authority_id')";
...
<body class="bg-gray-100">
    ...
    <section class="container mx-auto mt-6 text-gray-800">
        ...
        <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 max-w-2xl mx-auto"
            action="authority_announcement.php" method="POST">
